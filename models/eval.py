...
# Most logics come from https://github.com/jzhangbs/DTUeval-python/blob/master/eval.py
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scan', type=int, default=1, required=True)
    parser.add_argument('--mode', type=str, default='mesh', choices=['mesh', 'pcd'])
    parser.add_argument('--dataset_dir', type=str, default='.', required=True)
    ...
    vis_out_path = args.data.split(".ply")[0]
    ...
    write_vis_pcd(f'{vis_out_path}_d2s.ply', data_down, data_color)
    stl_color = np.tile(B, (stl.shape[0], 1))
    stl_alpha = dist_s2d.clip(max=vis_dist) / vis_dist
    stl_color[ np.where(above)[0] ] = R * stl_alpha + W * (1-stl_alpha)
    stl_color[ np.where(above)[0][dist_s2d[:,0] >= max_dist] ] = G
    write_vis_pcd(f'{vis_out_path}_s2d.ply', stl, stl_color)
